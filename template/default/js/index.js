	...
	Commands.prototype = {
		...
		pushState: function() {
			if (window.history && window.history.pushState)
				this.query ? history.pushState({}, "indexDB", "#!kw=" + this.query) :
				history.pushState({}, "indexDB", window.location.pathname);
		},
		//简单模版
		...
		createKeyworldsHTML: function(json, keywolds, islist) {
			...
			var name = json.name,
				...
				str = '';
			if (keywolds) {
				name = json.name.replace(reg, '<i class="kw">' + "$1" + "</i>");
				des = json.desc.replace(reg, '<i class="kw">' + "$1" + "</i>") || '';
			}
		...
		searchResult: function(islist) {
			...
			if (arr && arr.length && Object.prototype.toString.call(arr).indexOf('Array') > -1) {
				...
				for (; i < page_size; i++) {
					if (!arr[i]) break;
					if (self.isSreachIndexOF(arr[i].name, self.query) ||
						self.isSreachIndexOF(arr[i].desc, self.query)
					) {
