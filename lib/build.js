    ...
    if (res && res.render) {
        ...
        if (eeutils.isFile(renderFile)) {
            if (renderFile.endsWith(".json")) {
                ...
                let h = await renderMarkdown(txt, { pash: page.replace(".ejs", ""), category_id: 'other', category: '其他' });
                // console.log(h);
                res._render = h;
            }
        }
...
let renderMarkdown = async (mdstr, describe) => {
    ...
    if (_renderConfig.markedRender) {
        ...
        if (renderCode) {
            renderer.code = function (code, infostring, escaped) {
                let str = null;
                if (infostring == "json") {
                    str = formatPlugin(code, infostring);
                }
                let pluginRendered = str && str.length > 0;
                return pluginRendered ? str : _renderConfig.markedRender.code.format(code, infostring, escaped);
            }
        }
...
async function readPost(from_path, to_path, md_path, des_json) {
    ...
    let md_html = await renderMarkdown(markdownStr, describe);
    if (!md_html) {
