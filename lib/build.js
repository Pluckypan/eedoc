...
function build(commander, callback) {
	var _configFile = path.join(path_root, "config.json");
	...
	const themePath = path.resolve(template_path, theme);

	CreateDatajs('public/js/dt.js', function(arr) {
		// 拷贝 favicon.ico
		mkdirsSync(path.join(path_root, "public/img/"));
		var frompath = path.join(themePath, 'img/favicon.ico');
		var topath = 'public/img/favicon.ico'
		copy(frompath, topath)

		CreateJS(path.join(themePath, 'js/index.js'), 'public/js/index.js')

		CreateStyl(path.join(themePath, 'styl/index.styl'), 'public/css/index.css')
		...
		// 首页生成
		ReadTmpToHTML(path.join(themePath, 'index.ejs'), 'public/index.html', null, indexConfig);
		ReadTmpToHTML(path.join(themePath, 'list.ejs'), 'public/list.html', null, listConfig);
		// 文章批量生成
		arr.forEach(function(itm, idx) {
			var ejstpm = path.join(themePath, itm.p);
			var dep = path.join('public/c', itm.p);
			var md_path = path.join('command/', itm.p);
			ReadTmpToHTML(path.join(themePath, 'details.ejs'), dep + '.html', md_path + '.md', itm)
		});
		...
		if (callback) {
			...
			console.log(success("build success! total article : " + arr.length + ",now you can run 'eedoc -s'."));
		}
...
function ReadTmpToHTML(from_path, to_path, md_path, des_json) {
	...
	var cmtAppid = null,
		...
		gitalk = null;
	if (config.changyan &&
		config.changyan.appid && config.changyan.appid.length > 0 &&
		config.changyan.conf && config.changyan.conf.length > 0) {
		cmtAppid = config.changyan.appid;
		cmtKey = config.changyan.conf;
	}
	if (config.wumii && config.wumii.sitePrefix && config.wumii.sitePrefix.length > 0) {
		cmtSitePrefix = config.wumii.sitePrefix;
	}
	if (config.gitalk &&
		config.gitalk.clientID && config.gitalk.clientID.length > 0 &&
		config.gitalk.clientSecret && config.gitalk.clientSecret.length > 0 &&
		config.gitalk.repo && config.gitalk.repo.length > 0 &&
		config.gitalk.owner && config.gitalk.owner.length > 0 &&
		config.gitalk.admin && config.gitalk.admin.length > 0) {
		gitalk = config.gitalk;
	}
