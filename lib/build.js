...
renderer.heading = function (text, level) {
	if (/[\u4E00-\u9FA5]/i.test(text)) {
		const _id = text.toLowerCase();
		pushToc(level, _id);
		return '<h' + level + ' id="' + _id + '">' + text + '</h' + level + '>';
	} else {
		var _id = text.toLowerCase().replace(/[^\w]+/g, '-');
		pushToc(level, _id);
		return '<h' + level + ' id="' + _id + '">' + text + '</h' + level + '>';
	}
}
...
function hilight(code, lang, callback) {
	...
	var str = highlight.highlight(lang, code).value;
...
function build(commander, callback, _config) {
	...
	if (_renderConfig.markedRender) {
		...
		if (_renderConfig.markedRender.code) {
			renderer.code = function (code, infostring, escaped) {
				return _renderConfig.markedRender.code.format(code, infostring, escaped);
			}
		}
...
function customRenderTag(str, tag) {
	...
	if (end > start && start > 0) {
		try {
			config = JSON.parse(str.substring(start + 6 + tag.length, end + 1));
		} catch (e) {
			config = null;
		}
	}
 ...
 * 渲染自定义标签
 */
function renderingCustomTag(tag, str, data, page) {
...
function readPost(from_path, to_path, md_path, des_json) {
	...
	if (ctags) {
		...
		Object.keys(ctags).forEach((key) => {
			...
			do {
				...
				conf = wrap.config;
				if (conf != null) {
					let str = renderingCustomTag(key, markStr, conf, des_json);
					let start = wrap.start;
					let end = wrap.end;
					let len = markStr.length;
					markStr = markStr.substring(0, start) + str + markStr.substring(end, len);
					warn.pash = des_json.pash;
					warn.category_id = des_json.category_id;
	...
	marked(markdownStr, function (err, md_html) {
		if (err) return console.log(error('→ ' + new_md_path + " failed!"));
		// 生成 HTML
...
function createDatajs(dt_path) {
	...
	if (postTpl) {
		...
		path_arr.forEach(function (md_path, i) {
			...
			let postCfg = customRenderTag(str, 'eedoc');
			...
			var sDesc = findViewById('desc', false, true);
			if (sDesc && sDesc.length > 0) {
				des = sDesc;
			}

			var page = $('page').map(function (i, el) {
				var ele = $(this);
			...
			// 缩略图
			if (postCfg && postCfg.thumb) {
				json["thumb"] = postCfg.thumb;
			}
			// 色调
			if (postCfg && postCfg.color) {
				json["color"] = postCfg.color;
			}
			// 是否置顶热门或Banner
			...
			// 创建时间
			json["create_time"] = moment(postCfg && postCfg.create_time ? postCfg.create_time : stat.birthtime).format(
				"YYYY-MM-DD HH:mm:ss");
			// 更新时间
			json["update_time"] = moment(postCfg && postCfg.update_time ? postCfg.update_time : stat.mtime).format(
				"YYYY-MM-DD HH:mm:ss");
			if (postCfg && postCfg.category) {
				...
				json["category"] = _catName && _catName.length > 0 ? _catName : "default";
