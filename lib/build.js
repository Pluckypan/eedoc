	...
	CreateDatajs('public/js/dt.js', function(arr) {
		...
		if (arr.length > 0) {
			...
			arr.forEach(function(itm, idx) {
				...
				ReadTmpToHTML(path.join(themePath, _renderConfig.postTpl), dep + '.html', md_path + '.md', itm)
			});
		} else {
...
function ReadTmpToHTML(from_path, to_path, md_path, des_json) {
	...
	if (md_path) {
		...
		// iframe命中规则
		const iframeTplPath = path.join(themePath, _renderConfig.iframeTpl);
		var iframeTpl = null;
		if (eeutils.isFile(iframeTplPath)) {
			iframeTpl = fs.readFileSync(iframeTplPath).toString();
		}
		// gallery命中规则
		const galleryTplPath = path.join(themePath, _renderConfig.galleryTpl);
		var galleryTpl = null;
