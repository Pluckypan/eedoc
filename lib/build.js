...
function build(commander, callback, _config) {
	...
	// Markdown 图片的绝对路径
	if (config.docImage) {
		docImagePath = path.join(docPath, config.docImage)
		docImagePath = path.join(path_root, docImagePath)
	}
	...
	const _renderConfig = JSON.parse(fs.readFileSync(_renderConfigFile));

	CreateDatajs('public/js/dt.js', function(arr) {
		eedoc.total = arr.length;
		...
		_renderConfig.ejs.forEach(function(f, i) {
			...
			var topath = path.join('public', name);
			ReadTmpToHTML(frompath, topath, null, getPageConfig(f))
		});

		// 拷贝MarkDown图片
		if (eeutils.isDir(docImagePath)) {
			const dsti = path.join('public', config.docImage);
			eeutils.copyDir(docImagePath, dsti)
		}
		...
		if (postTpl) {
			...
			arr.forEach(function(itm, idx) {
				var ejstpm = path.join(themePath, itm.path);
				var dep = path.join(path.join('public', outPath), itm.path);
				var md_path = path.join(docPath, itm.path);
				ReadTmpToHTML(path.join(themePath, postTpl), dep + '.html', md_path + '.md', itm)
			});
		} else {
...
function CreateDatajs(dt_path, callback) {
	...
	path_arr.forEach(function(md_path, i) {
		...
		json["name"] = title;
		// 文章路径
		json["path"] = md_path.replace(/\.md$/, '').replace(path_md, '');
		// 文章描述
		json["desc"] = des;
		indexes.push(json)
	})
	...
	fs.writeFile(dt_path, jstr, 'utf8', function(err) {
		if (callback)
			callback(indexes);
	});
}
...
function readMDSync(filepath) {
	...
	for (var i = 0; i < files.length; i++) {
		...
		if (eeutils.isDir(path_c)) {
			// Markdown 目录下图片目录不解析
			if (!docImagePath || docImagePath != path_c) {
